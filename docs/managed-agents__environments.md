...
* `allowed_hosts` 指定沙箱可以访问的域名。请指定裸主机名或通配符模式（例如 `*.example.com`）。不要包含 URL 协议、端口或路径。
* `allow_mcp_servers` 允许对智能体上配置的 MCP 服务器端点进行出站访问，范围超出 `allowed_hosts` 数组中列出的主机。默认为 `false`。
* `allow_package_managers` 允许对公共软件包注册源（例如 PyPI 和 npm）进行出站访问，范围超出 `allowed_hosts` 数组中列出的主机。默认为 `false`。只要环境指定了 `packages`，就应将其设置为 `true`；否则请求将被拒绝并返回 400 错误，即使注册源主机已列在 `allowed_hosts` 中也是如此。
