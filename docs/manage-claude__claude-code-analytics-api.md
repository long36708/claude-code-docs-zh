...
description: 使用 Claude Code Analytics Admin API 以编程方式访问您组织的 Claude Code 使用情况分析和生产力指标。
---
...
Claude Code Analytics Admin API 提供对 Claude Code 用户每日聚合使用指标的编程访问，使组织能够分析开发者生产力并构建自定义仪表板。此 API 比基础的[分析仪表板](https://platform.claude.com/claude-code)提供更多细节，同时又没有 OpenTelemetry 集成的复杂性。

此 API 使您能够更好地监控、分析和优化您的 Claude Code 采用情况：
...
* **成本分析：** 查看按 Claude 模型细分的估算成本和令牌使用量
* **自定义报告：** 导出数据，为管理团队构建高管仪表板和报告
* **使用合理性论证：** 提供指标以在内部论证并扩大 Claude Code 的采用
...
<Note>
  **Claude Platform on AWS：** Claude Code Analytics API 目前不可用。请改为在 Claude Console 的 **Usage** 页面查看 Claude Code 使用情况。
</Note>

<Note>
  **Claude Enterprise 组织：** claude.ai 用户的 Claude Code 活动由 Claude Enterprise Analytics API 报告，该 API 使用 Analytics API 密钥而非 Admin API 密钥。请参阅 [Analytics API](https://platform.claude.com/docs/zh-CN/manage-claude/analytics-api) 以了解您的组织需要哪种 API 和密钥类型。
</Note>
...
<Tip>
  **为集成设置 User-Agent 标头**
  ...
  如果您正在构建集成，请设置 User-Agent 标头以帮助 Anthropic 了解使用模式：
...
* **用户级数据：** 每条记录代表一位用户在指定日期的活动
* **生产力指标：** 跟踪会话数、代码行数、提交数、拉取请求数和工具使用情况
* **令牌和成本数据：** 监控按 Claude 模型细分的使用量和估算成本
* **基于游标的分页：** 使用不透明游标以稳定的分页方式处理大型数据集
* **数据新鲜度：** 为保证一致性，指标最多有 1 小时的延迟

有关完整的参数详情和响应模式，请参阅 [Claude Code Analytics API 参考](https://platform.claude.com/docs/zh-CN/api/admin/usage_report/retrieve_claude_code)。
...
* **estimated\_cost.amount：** 此模型的估算成本（以美分计）
...
2. 如果响应中的 `has_more` 为 `true`，请在下一次请求中使用 `next_page` 值。
3. 持续进行，直到 `has_more` 为 `false`。
...
* **成本跟踪和分摊：** 监控支出模式并按团队或项目分摊成本
* **采用情况监控：** 识别哪些团队和用户从 Claude Code 中获得了最大价值
* **ROI 论证：** 提供具体指标以在内部论证并扩大 Claude Code 的采用
...
不可以，此 API 仅提供每日聚合指标。如需实时监控，请考虑使用 [OpenTelemetry 集成](https://code.claude.com/docs/en/monitoring-usage)。
...
`customer_type` 字段指示使用量来自 `api` 客户（按量付费 API）还是 `subscription` 客户（Pro/Team 套餐）。
